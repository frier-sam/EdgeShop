  ...
  return (
    <div className="px-4 py-3">
      <div className="flex items-start justify-between gap-3 mb-2.5">
        ...
        <p className="text-sm font-semibold text-gray-900 shrink-0">&#8377;{item.line_total.toFixed(2)}</p>
      </div>

      {!item.design && (
        <p className="text-xs text-gray-400">Plain item — no custom design on this line.</p>
      )}
      ...
      {renderableSides.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {renderableSides.map(({ geo, snapshot }) => {
            ...
            return (
              <div key={geo.side} className="flex gap-3 border border-gray-100 rounded p-2.5">
                {previewUrl ? (
                  <img
                    src={previewUrl}
                    alt={`${item.name} — ${SIDE_LABEL[geo.side] ?? geo.side}`}
                    className="w-16 h-16 object-cover rounded border border-gray-100 shrink-0 bg-gray-50"
                  />
                ) : (
                  <div className="w-16 h-16 rounded border border-gray-100 shrink-0 bg-gray-50" />
                )}
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-medium text-gray-700">{SIDE_LABEL[geo.side] ?? geo.side}</p>
                  <p className="text-xs text-gray-500">
                  ...
                  <p className={`text-xs ${dims.clamped ? 'text-amber-600 font-medium' : 'text-gray-500'}`}>
                  ...
                  <button
                    ...
                    className="mt-1.5 px-2 py-1 text-xs bg-gray-900 text-white rounded hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {isDownloading ? 'Rendering…' : 'Download print file'}
                  </button>
                </div>
              </div>
            )
