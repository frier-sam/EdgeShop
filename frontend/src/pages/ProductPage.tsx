...
export default function ProductPage() {
  ...
  const { data: recommendedData } = useQuery<{ products: ProductSummary[] }>({
  ...
  // Reset manual image override + selected size when product loads
  useEffect(() => {
    setManualImage(null)
    setSelectedSize(product?.sizes?.[0]?.label ?? null)
  }, [product])
  ...
  useEffect(() => {
    ...
    const frontUrl = product.sides.find(s => s.side === 'front')?.image_url ?? ''
    viewed.unshift({ id: product.id, name: product.name, price: product.base_price, image_url: frontUrl })
    // Keep max 5
  ...
  const frontSide = sides.find(s => s.side === 'front') ?? sides[0]
  const selectedSizeRow = sizes.find(s => s.label === selectedSize) ?? null

  const displayPrice = (product?.base_price ?? 0) + (selectedSizeRow?.price_delta ?? 0)
  const displayStock = selectedSizeRow ? selectedSizeRow.stock_count : (product?.stock_count ?? 0)
  const displayImage = manualImage ?? frontSide?.image_url ?? ''
  ...
  useEffect(() => {
    ...
    setMetaProperty('og:image', frontSide?.image_url ?? '')
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-16">
          ...
          <div>
            ...
            {/* Side thumbnails — front / back mockups */}
            {sides.length > 1 && (
              <div className="flex gap-2 mt-3">
                {sides.map((s) => (
                  <button
                    key={s.side}
                    onClick={() => setManualImage(s.image_url)}
                    className={`shrink-0 w-16 h-16 rounded-lg overflow-hidden border-2 transition-colors ${
                      displayImage === s.image_url ? 'border-[var(--color-primary)]' : 'border-transparent'
                    }`}
                  >
                    <img src={s.image_url} alt={`${product.name} ${s.side}`} className="w-full h-full object-cover" />
                  </button>
                ))}
              </div>
            )}
            ...
            {sizes.length > 0 && (
              <div className="mb-5">
                <p className="text-xs tracking-wider uppercase mb-2 font-semibold" style={{ color: 'var(--color-primary)' }}>
                  Size
                  {selectedSize && <span className="ml-2 normal-case font-normal opacity-60">— {selectedSize}</span>}
                </p>
