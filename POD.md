...
> **Status: conversion complete.** All 9 phases (0–9) are done. EdgeShop is a lean, single-purpose print-on-demand storefront: browse → customize on the product → preview → add to cart → buy, with a Fabric.js canvas editor, normalized print-area coordinates, a same-origin `/img/*` R2 proxy, server-authoritative pricing, on-demand 300 DPI print-file rendering in the merchant's browser, and a daily orphan-design GC. 132 tests pass (60 worker + 72 frontend), both `tsc` builds are clean, Fabric/fflate stay out of the main bundle, and docs (`README.md`, `DEPLOY.md`, `cloudflare-deploy.md`, `project.md`, deploy scripts) are rewritten for the current architecture. See §13 for the full decisions log.
>
> **2026-09-01 follow-up (post-conversion, not a new numbered phase):** made this repo deploy to a fresh Cloudflare account via Workers Builds Git integration with zero manual D1/R2/schema setup — self-bootstrapping schema (`0000_base_schema`), `wrangler.toml`'s `database_id` removed so automatic resource provisioning can create the D1 database, `wrangler` upgraded `^3.80.0` → `^4.45.0`, and `DEPLOY.md`/`cloudflare-deploy.md`/`README.md` rewritten accordingly. Worker tests now 83 (was 77 pre-follow-up; frontend unchanged at 100). See §13's 2026-09-01 rows for what was verified, what couldn't be (no Cloudflare login in this environment), and a real local-dev side effect this uncovered.
...
- [x] **9.1** Orphan-design GC: daily cron deleting `designs` with `order_id IS NULL` older than 30 days (setting: `design_retention_days`), plus their R2 previews. Upload (`uploads/`) GC deliberately deferred — see decisions log.
- [x] **9.2** Simplified `sitemap.xml` to products (slug-preferring); product-page JSON-LD.
- [x] **9.3** Tests — worker: design/side validation (`designs.ts`), `/img/*` proxy prefix/traversal guard, orphan-design GC selection logic, plus the existing price-recomputation coverage. Frontend: SVG sanitizer (new); cart line-key dedupe, print-area coordinate maths and the preview compositor's shared geometry were already covered pre-Phase-9 and reviewed for gaps, not re-padded.
- [x] **9.4** Rewrote `README.md`, `DEPLOY.md`, `cloudflare-deploy.md`, `project.md`, `CLAUDE.md`, `deploy.sh`, `scripts/setup.sh`, `.github/workflows-example/deploy.yml`, `frontend/public/robots.txt` for POD; archived `plan.md` under `docs/plans/2026-08-25-edgeshop-v1-v2-archive.md` with a pointer left at the old path.
- [x] **9.5** Bundle audit — confirmed Fabric and fflate absent from the main chunk (grep for library-internal tokens, not just import statements); full-tree straggler grep for deleted concepts came back clean (only historical-record mentions in migration DROP statements, POD.md itself, and the archived plan). Lighthouse was not run — no browser automation harness is set up in this repo and installing one for a one-off audit was judged out of scope for a cleanup phase; the bundle-size numbers above are the audit's live-verifiable substitute.
...
| 2026-08-25 | Phase 9.1: orphan-design GC ships design + R2-preview cleanup only; `uploads/` art-object GC is explicitly deferred, not implemented as a best-effort scan | Answering "is this uploads/ key referenced by ANY design, including paid ones" cheaply needs an index D1 doesn't have (`design_json` is a Fabric object graph, URLs nested at arbitrary depth) — the only D1-level answer is a full-table `LIKE` scan of every design (not just orphans) per upload key, on every GC run, in a Worker whose whole design philosophy is staying inside the free-tier CPU budget. If that scan is ever wrong, the failure mode is silent, permanent deletion of either in-progress customer artwork or a PAID ORDER'S print-file source image — exactly the asymmetric risk this task called out ("deleting art still referenced by a paid order is far worse than leaving an orphan file"). Full rationale lives as a code comment directly above `runOrphanDesignGC` in `worker/src/lib/gc.ts` so it survives independently of this log |
| 2026-08-25 | The orphan-design selection rule is a pure, dependency-free predicate (`isOrphanDesignExpired` / `selectOrphanDesignIdsFromRows` in `gc.ts`), separate from the D1/R2-touching orchestration (`runOrphanDesignGC`) | Mirrors the existing `lib/pricing.ts` pattern (`computeLine` etc.) that made the security-critical checkout math unit-testable without a mocked D1 — same technique applied here so the GC's "which rows qualify" rule has a real unit test (11 cases: boundary-exactly-30-days, one-ms-past, custom retention windows, unparseable `created_at`, and critically a row that's 200 days old but order-linked, which must NEVER be selected) rather than being verifiable only by waiting for a real cron or hand-inspecting SQL |
| 2026-08-25 | GC's SQL predicate (`created_at < datetime('now', '-' || ? || ' days')`) parses D1's `CURRENT_TIMESTAMP` format as explicit UTC in the JS-side pure predicate (`parseSqliteDatetimeUtcMs`), rather than handing the raw string to `new Date(...)` directly | D1/SQLite's `'YYYY-MM-DD HH:MM:SS'` timestamp carries no timezone suffix; `new Date('YYYY-MM-DD HH:MM:SS')` is parsed as LOCAL time by JS engines, which would make "older than N days" silently depend on the machine's timezone. Forcing UTC (`.replace(' ', 'T') + 'Z'`) keeps the JS-side unit test's notion of "expired" identical to what the SQL query actually returns — verified by seeding rows with `datetime('now','-45 days')` etc. directly into a local D1 and confirming the live SQL query returned exactly the two rows the pure-function tests predicted |
| 2026-08-25 | `design_retention_days` uses the same settings-allow-list pattern as every other numeric knob (`worker/src/routes/settings.ts`'s `NUMERIC_KEYS`, floored at 1, not 0) | A 0-or-negative retention would let the daily cron delete a design the same day it was created — plausibly wiping out a customer mid-"My Designs" re-edit or one who simply hasn't finished checkout yet. Matches the existing floor rationale already established for `print_dpi` (72) and `max_art_upload_mb` (1) rather than inventing a new validation shape |
| 2026-08-25 | Extracted `POST /api/designs`'s validation (`sides_used` shape, per-side customizable check, `design_json` size/JSON/object-not-array checks) into `worker/src/lib/designValidation.ts`, refactoring the route to call it rather than duplicating logic inline | Same rationale as the GC predicate extraction: this is the security-relevant boundary the task specifically flagged as untested, and the existing `pricing.ts` extraction pattern already proved that pulling validation out of the Hono handler into plain functions makes it cheaply unit-testable (24 new tests) without needing a mocked D1 or an HTTP request/response cycle |
| 2026-08-25 | While writing `designValidation.test.ts`, found and fixed a real (if minor) pre-existing gap: `design_json` that parses to a JSON ARRAY (e.g. `"[1,2,3]"`) passed the original `typeof parsedDesign !== 'object' \|\| parsedDesign === null` check, since `typeof [] === 'object'` | Not the intended shape (`design_json` is always `{version, front, back}`); a bare array would flow into `sides_used` validation and fail there anyway in practice, but the check itself was silently wrong about what "an object" means. Added `Array.isArray(parsed)` to the reject condition in `validateDesignJsonPayload` — a one-line fix surfaced directly by writing the test the task asked for, not a speculative hardening pass |
| 2026-08-25 | The `GET /img/*` prefix/traversal guard was extracted from an inline check in `index.ts` into `worker/src/lib/imgGuard.ts`'s `isAllowedImgKey`, purely so it has a unit test; behavior is unchanged | Live verification against a running `wrangler dev` (not just the unit tests) showed something worth recording: a literal `../` **path segment** never reaches this function's `.includes('..')` check at all in practice — the Workers runtime's own URL parsing collapses dot-segments per RFC 3986 before the request is even routed (`curl --path-as-is .../mockups/../../etc/passwd` resolves client- and server-side to `/etc/passwd`, which 404s via the SPA fallback, never touching `/img/*`). The app-level check earns its keep on the case that DOES survive normalization — `..` embedded WITHIN a single segment (e.g. `mockups/foo..bar.webp`), confirmed live to hit the route and get correctly rejected (404 from Hono's `c.notFound()`, not the SPA fallback) — and the prefix allow-list is the check that actually matters end-to-end, since it closes off any key outside `mockups/`, `uploads/`, `designs/` regardless of how a traversal payload is encoded |
| 2026-08-25 | `robots.txt`'s `Sitemap:` line changed from an absolute `https://YOUR-WORKER-URL/sitemap.xml` placeholder (patched by `deploy.sh` at deploy time) to a root-relative `/sitemap.xml`, and the corresponding "patch robots.txt with the worker URL" step was deleted from `deploy.sh` entirely | The storefront and the API have been same-origin since Phase 2 — a per-deploy sed patch for a same-origin URL was leftover complexity from the old two-origin (Pages + Workers) architecture. Also updated the stale `Disallow: /search` (route deleted in Phase 1) to `Disallow: /customize/` (the actual route that shouldn't be indexed — it's a mid-edit tool state, not a landing page) |
| 2026-08-25 | `deploy.sh` and `scripts/setup.sh` fully rewritten, not line-patched | Both scripts were functionally broken for the current architecture, not just stale in wording: they applied three numbered migration files that no longer exist (`0001_initial.sql`/`0002_v2_schema.sql`/`0003_abandoned_cart.sql` — the flagged `deploy.sh:178` bug), ran `wrangler deploy`/`wrangler secret put` from inside `worker/` even though `wrangler.toml` moved to the repo root during the POD conversion (Phase 2), deployed the frontend as a SEPARATE Cloudflare Pages project via `wrangler pages deploy` (the single-worker `[assets]` binding architecture replaced that entirely), and patched a `FRONTEND_URL` CORS setting and a "bot detection middleware" `WORKER_URL` Pages secret that don't exist anywhere in the current codebase. Root-caused further: the `sed_i "s/database_id = \"placeholder-replace-after-creation\"/.../"` patch target string doesn't exist in the committed `wrangler.toml` either (it holds the original dev database's real id) — the OLD scripts' database-id patching was silently a no-op, meaning a fresh clone following the old instructions would have deployed against someone else's D1 database without any error. The rewrite patches `wrangler.toml` by matching the `database_id = "..."` / `bucket_name = "..."` KEY via regex instead of a specific value, so it's correct regardless of what's currently committed |
| 2026-08-25 | Both deploy scripts and both deploy docs now spell out the admin-bootstrap step explicitly (first-ever customer auto-promotes to `super_admin`; later promotion via a documented `wrangler d1 execute --remote` command) as a named, numbered step rather than a passing mention | Called out directly in this task's brief as the thing that breaks a merchant's ability to reach their own admin panel if omitted (Phase 1 deleted the staff-management UI entirely — there is no invite flow to fall back on). Verified the underlying claim against `worker/src/routes/auth.ts` (the `isFirst` check and `role='super_admin'` update) rather than assuming POD.md's own text was still accurate post-refactor |
| 2026-08-25 | `.github/workflows-example/deploy.yml` rewritten to a single job (`npm install` + `npm run deploy` from the repo root) instead of two jobs (a worker deploy job, then a separate `wrangler pages deploy dist` job depending on it) | Same root cause as the deploy scripts — the two-job version deploys to a Cloudflare Pages project that the single-worker-with-assets architecture replaced back in the original build-out; kept the file under `workflows-example/` (not `workflows/`) as already established, so a fork doesn't start deploying to someone else's Cloudflare account the moment it's pushed |
| 2026-08-25 | Product JSON-LD's `offers.price` is `product.base_price` alone — never `displayPrice` (which already folds in a selected size delta) and never base_price plus an assumed print fee | For an `is_customizable` product the real checkout total also depends on which side(s) the shopper actually designs (§7.1's per-side `print_fee`), which isn't knowable until they've used the editor — advertising a guessed final price in structured data is exactly the "price the customer cannot actually pay" failure mode this task warned against. `base_price` is the one number that's always true: the real floor price, honestly representing "at least this much," with the true total surfaced to the shopper in the on-page price breakdown and enforced authoritatively server-side at checkout regardless of what the JSON-LD says |
| 2026-08-25 | `sitemap.xml` now prefers `product.slug` over the numeric id when generating each product URL (`/product/<slug-or-id>`), with no frontend routing change required | `GET /api/products/:id` (routes/products.ts) already resolves either a numeric id or a slug via a regex check — this was true before Phase 9 and untouched by it — so the sitemap only needed to change which value it emits, not how the storefront resolves it. Slugs are nullable in the schema (some products may have none), so the fallback to `String(p.id)` is load-bearing, not decorative |
| 2026-09-01 | Cloudflare zero-manual-setup deploy task: reproduced the empty-DB failure BEFORE writing any fix, against a scratch (non-destructive) `--persist-to` D1, rather than assuming `migrate.ts` was incomplete | Got the actual verbatim error (`D1_ERROR: no such table: products: SQLITE_ERROR`, thrown from inside `0012_rewrite_image_urls.sql`'s first `UPDATE products`) rather than a guessed one — confirms the root cause precisely: `MIGRATIONS` only ever held incremental migrations (0012+), nothing created the base tables |
| 2026-09-01 | Base schema is generated into `worker/src/lib/schemaSql.generated.ts` from `worker/migrations/schema.sql` by `worker/scripts/generate-schema-sql.mjs` (`npm run generate:schema`, wired as a `predev`/`pretest`/`predeploy` hook), not hand-duplicated and not imported natively as a bundler text-asset | Wrangler's esbuild bundler *can* do custom `[[rules]] type="Text"` globs for `.sql` (confirmed by reading `DEFAULT_MODULE_RULES` in wrangler's own bundle — `.sql` isn't a default text-type extension, but a custom rule is supported), but that import would ALSO need to satisfy `tsc --noEmit` (no ambient `*.sql` module declared) and plain `vitest` (this repo's vitest has no `vitest.config.ts`/pool-workers wiring, so it never consults wrangler's module rules at all) — three different tools would each need their own accommodation for one raw-text import. The generated-`.ts`-constant approach needed none of that: it's plain, valid TypeScript everywhere. Not exhaustively proven that the native-import route fails end-to-end, only that it's more fragile across this repo's specific three-tool toolchain — recorded honestly as a reasoned choice, not an exhaustive comparison |
| 2026-09-01 | `migrate.test.ts`'s drift check reads `schema.sql` a SECOND, independent way — Vite/Vitest's built-in `?raw` suffix import (`worker/src/lib/sql-raw.d.ts` ambient-declares it) — rather than `node:fs` | The worker's `tsconfig.json` deliberately restricts `"types"` to only `@cloudflare/workers-types` (no Node globals, since the Workers runtime's `fetch`/`Response`/etc. types would otherwise collide with `@types/node`'s). Adding `@types/node` just for one test file's `readFileSync`/`import.meta.url` would have reopened that isolation repo-wide; `?raw` is a zero-config Vite/Vitest feature that needed only a 6-line ambient `.d.ts`, and never reaches wrangler's own bundler (`index.ts`'s real dependency graph never imports it) |
| 2026-09-01 | `runMigrations`' in-memory `applied` Set is updated immediately after `0000_base_schema` runs — adding all 15 legacy migration names it just wrote to `_migrations` via `INSERT OR IGNORE` — not left to be picked up on a later run | Found by reasoning through the fresh-DB code path before trusting it, then confirmed live: `applied` is computed ONCE from a `SELECT` before the loop starts, so without this fix, `0012_rewrite_image_urls.sql` etc. would immediately try to re-run against the schema `0000_base_schema` just finished creating, in the SAME cold start — and fail, since `0013_pod_reset.sql` selects a `products.image_url` column the fresh POD-shaped `products` table never had. The legacy names are extracted from `schema.sql`'s own bookkeeping `INSERT` statement at module load (`extractLegacyMigrationNames`), not a second hand-written list, so there's exactly one place that list is written down |
| 2026-09-01 | wrangler upgraded `^3.80.0` → `^4.45.0` (resolved to 4.86.0 via `npm install`); no code changes were needed anywhere in `worker/` or `frontend/` to accommodate it | Ran the full toolchain after the bump — `tsc --noEmit` (worker + frontend), `vite build`, `vitest run` (worker: 83/83 incl. 6 new; frontend: 100/100, untouched) — all clean. The only externally-visible change was `npx wrangler d1 execute`'s local-database resource-location banner text and a `(local)`-mode `metadata.sqlite` bookkeeping file that didn't exist under wrangler 3 — neither affects this app |
| 2026-09-01 | `database_id` removed from `wrangler.toml`'s `[[d1_databases]]` (kept `binding`/`database_name`) exactly as the task specified, and the resulting local-dev side effect was tested rather than assumed away | Verified empirically, by isolating the variable (ran `wrangler dev` against the SAME default `--persist-to` path with `database_id` present vs. absent): Miniflare's local D1 file identity is derived partly from `database_id`, so removing it makes `wrangler dev` resolve to a DIFFERENT (new, empty) local sqlite file — a ONE-TIME identity change, confirmed stable/deterministic across repeated runs (not a new file every run), and the original file was confirmed byte-identical and untouched throughout (`sqlite3` queried directly: 3 products / 2 orders / 1 customer / unchanged). Documented as a one-time transition in DEPLOY.md's Local Development section, with both a reseed option and a "keep your old id locally, uncommitted" option |
| 2026-09-01 | R2's `[[r2_buckets]]` binding needed NO change for automatic provisioning | Unlike D1, R2 bindings in `wrangler.toml` never carry a resource id to begin with — only `binding` and `bucket_name` — so the binding was already in the exact shape Cloudflare's provisioning docs describe as the trigger |
| 2026-09-01 | `deploy.sh` and `scripts/setup.sh`'s `database_id`-patching `sed` commands were fixed (delete-then-`awk`-insert-after-`database_name`, tested against a scratch copy of the real `wrangler.toml`, including a second-run idempotency check) rather than left as-is | Both scripts' original `sed "s/^database_id = \".*\"/.../"` REPLACE-only pattern silently matched nothing once `database_id` was removed from `wrangler.toml` — the scripts would have failed loudly (their own `grep -q ... \|\| die` check catches it) rather than corrupting anything, but "loudly broken" is still broken; these are the documented manual-fallback / CLI deploy paths this same task asked to keep working |
| 2026-09-01 | Discovered (not part of this task's original ask, fixed anyway where cheap, flagged where not) that `JWT_SECRET` was documented in `README.md`/`DEPLOY.md`/`cloudflare-deploy.md`/`deploy.sh`/`scripts/setup.sh` as a required Worker secret, but the `Env` type (`worker/src/index.ts`) and every route that signs/verifies a JWT (`worker/src/lib/auth.ts`'s `getOrCreateJwtSecret`) never read `env.JWT_SECRET` at all — the signing key is generated once and persisted in the `settings` table instead | Fixed the three docs (they're this task's explicit scope and actively mislead a fresh-account deployer into thinking a step is required that does nothing). Left `deploy.sh`/`scripts/setup.sh`'s `wrangler secret put JWT_SECRET` calls in place — harmless (the value is simply never read) but outside this task's explicit scope of "D1/R2/schema automation + DEPLOY.md accuracy"; flagged to the user rather than silently left both broken-looking and unfixed |
| 2026-09-01 | `DEPLOY.md` restructured with "Deploy from Git" as the lead path (auto-provisioning + self-bootstrap) and the pre-existing dashboard/CLI walkthroughs demoted to explicitly-labeled Options B/C plus a "Manual fallback"; `cloudflare-deploy.md` rewritten as a shorter quickstart through the same Git path that defers detail to `DEPLOY.md` rather than re-diverging into a second full walkthrough | The two files already coexisted as "comprehensive reference" vs. "streamlined walkthrough" (per `POD-UI2.md`'s file inventory) — keeping that shape avoided a bigger restructure, while making `DEPLOY.md` the single place the fuller caveats (open-beta status, what's verified vs. not, manual fallback, local-dev one-time reset note) live once, so the two files can't drift back out of sync with each other the way `JWT_SECRET` had |
| 2026-09-01 | Every "automatic provisioning works in CI" claim in the rewritten docs is phrased as unverified/open-beta, with the verified claim scoped precisely to "the schema self-bootstraps against an empty D1" | No Cloudflare login was available in this environment (`wrangler whoami` fails), so real account-level D1/R2 provisioning inside a Workers Builds run could not be exercised at all — only reasoned about from `wrangler.toml`'s binding shape and Cloudflare's changelog post. Everything that WAS exercised (empty-DB failure reproduction, self-bootstrap on a scratch DB, idempotency on a second cold start, non-destructive verification against the real local DB, the full `tsc`/`vitest`/`vite build` toolchain post-upgrade) is called out as such, with the actual commands/output, rather than folded into the same confidence level as the unverified part |
| 2026-09-02 | Bug 1 (mobile: page opens scrolled to the previous page's offset) fixed with a `ScrollToTop` component (`frontend/src/components/ScrollToTop.tsx`) mounted inside `<BrowserRouter>`, using `window.scrollTo({top:0,left:0,behavior:'instant'})` on pathname change, skipping `/customize/*` and `POP` navigations | `behavior:'instant'` is load-bearing, not decorative — caught live against `wrangler dev`, not by the unit test: `index.css` sets a global `html{scroll-behavior:smooth}`, and per the CSSOM View spec `behavior:'auto'` (the first, wrong, attempt) means "defer to the element's own `scroll-behavior`" — it does NOT force instant. With `'auto'`, a long `/shop` page visibly glided back to the top over ~300ms instead of jumping, i.e. it "fixed" the bug by replacing an invisible jump-to-footer with a visible animated scroll, failing the explicit "no visible jump" requirement. `'instant'` is the value that actually overrides page CSS. Verified with real Playwright/Chromium clicks (not just `window.scrollTo` assertions): footer→product-click→scrollY=0 on 5/5 runs at both 390×844 and 1280×900; `/shop?category=` filtering does not move `scrollY`; POP (back) is left to the browser's native `scrollRestoration:'auto'` (never overridden) since nothing in the app sets it to `'manual'` — confirmed live that back-navigation restores close to the pre-navigation offset, not 0 |
| 2026-09-02 | Bug 2 (mobile: the auto-opened properties Sheet covers the very object/handles it's meant to edit) fixed by (a) no longer auto-opening the Sheet on selection, (b) a new always-visible `SelectionActionBar` (`frontend/src/editor/components/SelectionActionBar.tsx`) — delete/duplicate/layer-forward/layer-back/colour-swatch/Edit, every target ≥44px, reusing existing `objectsApi`/Fabric-object mutation patterns already used by `PropertiesPanel` rather than inventing new ones, and (c) the full Sheet, when deliberately opened via "Edit", now shrinks the stage instead of overlaying it | (c)'s mechanism is deliberately dumb: `CustomizerEditor`'s stage wrapper gets `paddingBottom` equal to the Sheet's current snap height (`Sheet.tsx` gained an additive, backward-compatible `onSnapChange` callback so the caller knows peek vs. full) — this makes EditorStage's EXISTING `ResizeObserver`-driven geometry recompute do all the work, the same machinery a window resize already goes through, rather than adding a second stage-sizing code path. Verified live: selecting a fresh text/image/shape never opens the Sheet (action bar appears instead); dragging the object with the mouse works (no overlay intercepts pointer events); tapping "Edit" shrinks the canvas element (measured: 225×222 at y=185 → 159×158 at y=53) to stay fully above the Sheet's measured top edge; closing the Sheet (backdrop click) restores full stage size AND keeps the object selected (action bar still visible) rather than deselecting; Delete removes the object in one tap for text, image and shape. Confirmed the action bar never renders at 1280×900 (desktop keeps the existing always-open right rail) |
| 2026-09-02 | Bug 3a (objects can be inserted bigger than the bleed rect, making handles unreachable from the start) fixed with a small pure module, `frontend/src/editor/fitObject.ts`'s `computeInsertFitScale`, applied to every insert path (`useEditorObjects.ts`'s `addText`/`addImage`/`addShape`) | Deliberately independent of Bug 3b's gutter (per the task brief) and framed around the bleed rect's SMALLER dimension so it also catches the case a naive width-only cap (the pre-existing `addImage` `maxDisplayWidth`) misses: a tall/narrow image whose width is small but height dwarfs the print area. Scaling happens around the object's own centre (every `fabric/objects.ts` factory already uses `originX/originY:'center'`), so "and centre it" from the bug report falls out for free — no separate re-centre step was needed. 7 unit tests, including the width-only-cap-miss case reproduced directly |
| 2026-09-02 | Bug 3b (small print area ⇒ an object's resize/rotate handles render outside the canvas element and are unusable) fixed via POD.md §5.2's own documented fallback — grow the canvas ELEMENT by a fixed gutter, `frontend/src/editor/canvasGutter.ts` — landing on `HANDLE_GUTTER_PX = 64`, not the "24-32px" originally suggested | Checked fabric@6.9.1's actual control geometry (`node_modules/fabric/dist/src/controls/commonControls.mjs`) before picking a number: the rotation control has a FIXED `offsetY:-40` (independent of object size or how much fit-margin Bug 3a left) plus a 24px `touchCornerSize` hit area, i.e. a ~52px minimum reach in the worst case, in whichever of the four directions the object currently happens to be rotated toward (so all four sides need the same margin, not just "above"). Even 56px measured with zero spare px in live Chromium on a genuinely tiny mobile print area (product id 8, "White Polo", a 2-inch chest-logo-sized print area) — bumped to 64px for real headroom. Final measured margins from the selection UI (border+handles, isolated in a real screenshot via a blue-tint colour heuristic tuned to fabric's actual `cornerColor` `rgb(178,204,255)` including its antialiased/blended edge pixels, not a plain colour-distance match) to the canvas element's edge: mobile 390×844 — left 61.3px, top 29.9px, right 61.7px, bottom 67.1px; desktop 1280×900 — left 65.6px, top 42.0px, right 66.4px, bottom 69.0px. All positive with real margin, not a knife-edge. Both the resize handle and the rotation handle were confirmed genuinely interactive (not just visually present) by dragging each and diffing the rendered canvas pixels before/after |
| 2026-09-02 | Bug 3b's print-fidelity safety net: `canvas.getWidth()/getHeight()` now report the gutter-INCLUSIVE size, so every caller that used to treat that as "the bleed rect's size" was found and switched to a new `onBleedSizeChange`/`getBleedSize` plumbing carrying the PURE bleed size instead — `useEditorObjects.ts`'s `canvasCenter()` (new-object placement) and `addImage`/`addShape` sizing, plus `CustomizerEditor.tsx`'s live DPI scan and the add-to-cart canonicalization step (`perSide[activeSide] = {..., width: bleedSizeRef.current.width, ...}`, previously `...getCanvasSize(canvas)`) | This is the actual "silently destroy print fidelity" risk the task warned about — not the visual gutter itself. `EditorStage.tsx`'s geometry effect was restructured (not the frozen `fabric/canvas.ts` — same two calls, `setCanvasDimensionsRaw`/`resizeCanvasScaled`, just sequenced differently) into "un-grow to the PURE bleed size → let the frozen `resizeCanvasScaled` compute its ratio against a canvas that's never gutter-polluted → re-grow by the gutter as a final raw (non-rescaling) step" on every geometry pass, so `resizeCanvasScaled`'s plain `nextWidth / canvas.getWidth()` ratio can never be contaminated by a gutter that doesn't scale proportionally with the bleed rect. Proved exact (not just "close") two ways: (1) a mock-canvas integration test (`gutterResizeIntegration.test.ts`) drives the REAL, unmodified `resizeCanvasScaled` through this exact sequence and asserts the resulting object transform is bit-identical to the same resize performed with no gutter at all; (2) live, a design built through the actual gutter-aware editor (insert text on product 8, drag-resize, "Add to cart") produced a canonical object centred at EXACTLY normalized (0.500, 0.500) — the mathematically exact expected value for a centred object, which an asymmetric X/Y bug could not have produced by coincidence — and the admin-rendered print file's content bounding box centred at (0.501, 0.498), agreeing with both the design_json and the customer preview image (fetched and visually compared) |
| 2026-09-02 | Bug 3's non-negotiable print-registration check (Phase 8's methodology, reproduced) confirms `admin/print/**`/`editor/preview.ts`/`editor/designSchema.ts` needed ZERO changes, as predicted | `editor/preview.ts` (the one frozen file the task flagged as possibly needing a touch) turned out to need none: it already builds its OWN fresh `fabric.StaticCanvas` sized to the canonical (reference-scale) bleed rect from `design_json` alone — it never reads the live, gutter-inclusive DOM canvas element at all, so it was never exposed to the gutter in the first place. Reproduced the exact Phase 8 scenario (100×100 rect in an 800×800 canonical canvas at normalized (0.375,0.25,0.125,0.125), `print_width_in=10`, `print_dpi=300`, via real `POST /api/designs` + `POST /api/checkout` + a real admin-UI "Download print file" click, decoded via the same browser's own `data:`→`<canvas>`→`getImageData`, no external PNG library): output PNG exactly 3000×3000 (`round(10in×300dpi)`), background alpha 0 at 5 sample points, centre pixel exactly `rgba(255,0,0,255)`, bounding box normalized to x=0.37467, y=0.24967, w=0.12567, h=0.12567 — every axis within 0.001 (0.1%) of the expected value, i.e. tighter than Phase 8's original 0.3% figure (that run's own residual was attributed to "grid-sampling granularity", and a solid rect samples more precisely than antialiased text) |
| 2026-09-02 | A pre-existing, unrelated bug was found while verifying Bug 3 and deliberately NOT fixed (out of this task's 3-bug scope, and touches the frozen `admin/print/**`) | `frontend/src/editor/fonts.css` (the self-hosted `@font-face` declarations, POD.md §5.5) is imported only by `CustomizerEditor.tsx` — never by the admin order-detail bundle that renders print files. A design using any non-default font (e.g. the default "Poppins") therefore renders correctly in the customer's own preview (same page, font-face present) but falls back to the browser's generic serif in the admin-downloaded print file, confirmed live by rendering the SAME design's customer preview and admin print file side by side: identical position/scale/transparency (see the row above), but visibly different typeface. This is a glyph-rendering gap, not a geometry/print-fidelity regression from this task's changes — `renderPrintFile.ts` and `fonts.ts` are both untouched and this would have reproduced identically before Bugs 1-3 were touched. Flagged here rather than fixed silently, per this branch's "ask before expanding scope" rule |
| 2026-09-02 | Live verification data was cleaned up, not left behind | Two scratch orders/designs and their R2 preview objects (created via real `POST /api/checkout` + admin "Download print file" runs against `wrangler dev`'s local D1/R2, needed because the print-registration check has to go through the real checkout pricing/stock path) were deleted afterward, and the two stock-count decrements checkout triggered as a side effect (`product_sizes` id 18: 100→99, `products` id 8: 5→4) were reverted. Confirmed by direct query after cleanup: exactly the original 3 products / 2 orders / 1 design, with every stock count back to its pre-verification value |
