...
- [ ] **6.1** Add `fabric@^6` and lazy-load it via `React.lazy` on `/customize/:productId` only. Verify the main bundle is unchanged.
- [ ] **6.2** `EditorStage`: two-layer mockup + canvas + scrim + guides (§5.2), responsive resize preserving normalized coordinates.
- [ ] **6.3** `useEditorObjects`: add/select/transform/delete/duplicate/reorder, undo-redo ring buffer (30 steps).
- [ ] **6.4** Text tool — curated self-hosted fonts, size, colour, weight, alignment, letter-spacing.
- [ ] **6.5** Image tool — drop/pick, client resize guard, SVG sanitize, immediate `POST /api/uploads/art`, low-DPI warning badge (§5.1).
- [ ] **6.6** Shape tool — rect, circle, triangle, star, line with fill/stroke.
- [ ] **6.7** Side tabs holding independent per-side state; live price footer.
- [ ] **6.8** Preview state: guides and selection hidden, front/back toggle, Back-to-edit / Add-to-cart.
- [ ] **6.9** Mobile layout — bottom tool sheet, pinch-zoom on the stage, 44px touch targets.
- [ ] **Acceptance:** a design with text + an uploaded image + a shape can be built on both sides, art past the print edge is visibly trimmed, and preview shows zero guide lines.

### Phase 7 — Design persistence, cart, checkout

- [ ] **7.1** `POST /api/designs`, `GET /api/designs/:id`, `PUT /api/designs/:id/preview` in the worker.
- [ ] **7.2** Preview compositor (§5.6) and the add-to-cart sequence (§3.5).
- [ ] **7.3** Cart drawer with design thumbnails, per-side fee breakdown and Edit-design deep link back into the editor.
- [ ] **7.4** Rewrite `checkout.ts` with full server-side recomputation (§7.3) and design→order linking; delete the discount and shipping-zone logic.
- [ ] **7.5** Update the order-confirmation email template to embed design previews.
- [ ] **Acceptance:** a tampered `total_amount` is rejected with `price_mismatch`; a completed order's `items_json` carries the correct per-side fees and design id.

### Phase 8 — Admin fulfilment

- [ ] **8.1** `GET /api/admin/orders/:id` returns `design_json` + side geometry per line.
- [ ] **8.2** `PrintFileRenderer` — lazy Fabric, `loadFromJSON`, font preload, `toDataURL` at `print_dpi`, download as `<order>-<line>-<side>.png`.
- [ ] **8.3** Order detail design panel: preview, dimensions readout, effective DPI, per-side and whole-order download.
- [ ] **8.4** Trim `AdminDashboard` to orders today / revenue / pending fulfilment / low stock.
- [ ] **Acceptance:** the downloaded PNG is transparent, exactly `print_width_in × print_dpi` wide, and its artwork registers pixel-for-pixel with the customer preview.
